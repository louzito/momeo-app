...
// Le virement crée une vraie commande Sylius. Aucun moyen de paiement simulé
// n'est proposé : une réservation publique doit toujours être persistée.
...
// Sans virement actif chez le centre : aucun moyen de creer une
// vraie commande -> on bloque avant l'appel API plutot que de laisser passer
// silencieusement un cheque cadeau mock.
const blocked = computed(() => methodsLoaded.value && !canBankTransfer.value)
  ...
  >
    <div class="max-w-lg">
      ...
      <p class="mt-3 flex items-center justify-center gap-1 text-xs text-slate-400">
        🏦 Commande reelle (Sylius) · paiement a reception du virement
      </p>
    </div>
  </CheckoutLayout>
</template>
