...
onMounted(async () => {
  ...
  if (sso === 'error') {
    error.value = 'Le lien de connexion a expiré. Ouvrez à nouveau votre espace depuis Momeo.'
    return
  }
  ...
  try {
    ...
    error.value = 'La connexion automatique a expiré. Ouvrez à nouveau votre espace depuis Momeo.'
  } finally {
      ...
      <section class="flex min-h-[30rem] flex-col justify-between bg-slate-950 p-8 text-white sm:p-12">
        <div>
          <RouterLink :to="{ name: 'home' }" class="inline-flex items-center gap-3 text-white">
            <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-300 font-display text-xl font-black text-slate-950">M</span>
            <span class="font-display text-xl font-extrabold tracking-tight">Momeo</span>
          </RouterLink>
          ...
          <p class="mt-5 max-w-md text-base leading-7 text-slate-300">
            Gérez vos prestations, votre agenda, vos réservations et votre boutique depuis votre espace Momeo.
          </p>
        </div>
      ...
      <section class="flex items-center p-8 sm:p-12">
        <div class="w-full">
          ...
          <h2 class="mt-2 font-display text-3xl font-bold text-slate-950">
            {{ automaticLogin ? 'Connexion à votre espace…' : 'Connexion à Momeo' }}
          </h2>
