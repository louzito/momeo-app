...
TPL=$(bin/console todatempo:tenant:list --status=template --count)
if [ "$TPL" -lt 1 ]; then
  echo "ERREUR : aucune BDD template. Lance d'abord template-init.cmd." >&2
  exit 1
fi
CUR=$(bin/console todatempo:tenant:list --status=pool --count)
echo "Pool actuel : $CUR / $N"
while [ "$CUR" -lt "$N" ]; do
  bin/console todatempo:tenant:pool-add
  CUR=$((CUR + 1))
done
