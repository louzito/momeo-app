...
#[AsCommand(name: 'skybook:tenant:initialize', description: 'Initialise les données Sylius minimales du tenant courant')]
final class TenantInitializeCommand extends Command
{
    ...
    {
        $email = getenv('SKYBOOK_ADMIN_EMAIL');
        $password = getenv('SKYBOOK_ADMIN_PASSWORD');
        if (!is_string($email) || filter_var($email, \FILTER_VALIDATE_EMAIL) === false || !is_string($password)) {
            $output->writeln('<error>SKYBOOK_ADMIN_EMAIL et SKYBOOK_ADMIN_PASSWORD doivent être fournis par l’environnement.</error>');
